...
export class IndexPage implements OnInit {
  ...
  async settings(event, i, dateindex) {
    ...
    const modal = await this.modalController.create({
      ...
      componentProps: { iduser: event, index: i, date: dateindex, currentUserId: this.currentUser.id, EventsID: this.EventsID },
  ...
  listpresence() {
    ...
    this.api.getPresence().then(data => {
      ...
      this.users = test.personnels;
      console.log('users', this.users)
      this.api.dismissFn();
      if (this.users) {
        this.users.forEach(personel => {
          if (personel.id == this.currentUser.id) {
            personel.evenements.forEach(element => {
              this.EventsID.push(element.id)
              console.log('EventsID', this.EventsID)
            });
            console.log('EventsID', this.EventsID)
          }
          ...
          for (var key in personel.evenements) {
            ...
            if (!Object.keys(personel.evenements[key].finParticipation).includes(String(this.currentUser.id)) || this.currentUser.id != personel.id) {
              ...
              this.schedule.forEach(s => {
                let presenceEvent = { "value": null, "nom": null, "debut": null, "description": null, "lieu": null, "type": null, "id": null, "idusers": null, "idusersExit": null, "salle": null, "email": null };
                if (events.includes(s)) {
                  presenceEvent.value = true;
                  presenceEvent.nom = personel.evenements[key].nom;
                  presenceEvent.debut = personel.evenements[key].debut;
              ...
              console.log('evenementsList', evenementsList)
          ...
          personel.TeleTravail.forEach(teletravail => {
            ...
            this.schedule.forEach(s => {
              let TeleTravail = { "value": null, "id": null, "nbJours": null, "debut": null, "fin": null, "reprise": null, "valide": null };
              if (travail.includes(s)) {
                ...
                TeleTravail.valide = teletravail.valide;
          ...
          infoUser.teletravail = demandeTeleTravail;
      ...
      console.log('listInfoUserTrier', this.listInfoUserTrier)
