...
@Component({
  ...
  templateUrl: './staff-presence.page.html',
  styleUrls: ['./staff-presence.page.scss'],
})
export class StaffPresencePage implements OnInit {
  datearray = [];
  presence;
  ...
  listpresence() {
    ...
    this.api.getPresence().then(data => {
      ...
      this.guests = test.guests;
      this.api.dismissFn()
      if (this.guests) {
        this.guests.forEach(personel => {
