...
@Component({
  selector: 'app-sfm-cameroun-presence',
  templateUrl: './sfm-cameroun-presence.page.html',
  styleUrls: ['./sfm-cameroun-presence.page.scss'],
})
export class SfmCamerounPresencePage implements OnInit {
  ...
  constructor(private authService: AuthtificationtokenService, private alert: AlertController,
    private modalController: ModalController, private fcm: FCM, private menuCtrl: MenuController, private storage: Storage, private toastController: ToastController, private api: RestapiService) {
  ...
  listpresence() {
    ...
    this.api.getPresence().then(data => {
      ...
      console.log("presenceeeeee :", test);

      this.SfmCameroun = test.SfmCameroun;
      this.api.dismissFn()

      if (this.SfmCameroun) {
        this.SfmCameroun.forEach(personel => {
          ...
          personel.demandeConges.forEach(conge => {
            ...
            while (this.state) {
              // console.log("fin conge : ",moment(conge.dateFinConge).format('L'))
              // console.log("debut conge :",moment(conge.dateFinConge).format('L'))
